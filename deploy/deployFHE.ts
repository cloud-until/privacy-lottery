...
async function main() {
  ...
  // Deploy the plain PrivacyLotteryFHE contract
  const AdvancedAnonymousCheckInFactory = await hardhatEthers.getContractFactory("PrivacyLotteryFHE", wallet);
  ...
  console.log("PrivacyLotteryFHE contract deployed at:", deployedAddress);
  ...
  if (!fs.existsSync(frontendConfigDir)) {
    ...
    try {
      const artifactPath = path.join(
        ...
        "artifacts",
        "contracts",
        "PrivacyLotteryFHE.sol",
        "PrivacyLotteryFHE.json"
      );
      ...
      fs.copyFileSync(artifactPath, path.join(targetAbiPath, "PrivacyLotteryFHE.json"));
      console.log("Copied ABI to frontend/web/src/abi/PrivacyLotteryFHE.json");
    } catch (e) {
      console.warn(
        "Failed to copy ABI automatically. Please copy artifacts/.../PrivacyLotteryFHE.json manually to frontend/web/src/abi/PrivacyLotteryFHE.json",
        e
      );
    }
  }
