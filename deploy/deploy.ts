...
async function main() {
  ...
  // Deploy the plain PrivacyLottery contract
  const PrivacyLotteryFactory = await hardhatEthers.getContractFactory("PrivacyLottery", wallet);
  const factory = await PrivacyLotteryFactory.deploy();
  await factory.waitForDeployment();

  const deployedAddress = (factory as any).target || (factory as any).address;
  console.log("PrivacyLottery contract deployed at:", deployedAddress);
  ...
  if (!fs.existsSync(frontendConfigDir)) {
    ...
    try {
      ...
      fs.copyFileSync(artifactPath, path.join(targetAbiPath, "PrivacyLottery.json"));
      console.log("Copied ABI to frontend/web/src/abi/PrivacyLottery.json");
    } catch (e) {
      console.warn(
        "Failed to copy ABI automatically. Please copy artifacts/.../PrivacyLottery.json manually to frontend/web/src/abi/PrivacyLottery.json",
        e
      );
    }
  }
